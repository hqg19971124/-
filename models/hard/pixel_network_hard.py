...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, BatchNorm=None):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, dilation, downsample=downsample, BatchNorm=BatchNorm))
...
class DeepLab(nn.Module):
    ...
    def forward(self, input):
        ...
        x1 = self.A_aspp(x)
        x2 = self.B_aspp(x)
        x1 = self.A_decoder(x1, low_level_feat)
        x2 = self.B_decoder(x2, low_level_feat)
